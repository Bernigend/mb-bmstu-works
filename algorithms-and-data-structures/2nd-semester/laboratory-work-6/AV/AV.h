...
protected:
    ...
    void charToBinaryTree(const std::string& rawString, BinaryTree*& node)
    {
    ...
    static void toLine(std::string& outString, BinaryTree* node)
    {
        if (node == NULL) return;
        char symbol;
...
public:
    explicit AV(const std::string& rawString) : values(NULL)
    {
        this->charToBinaryTree(rawString, this->values);
    }

    friend std::ostream& operator<< (std::ostream& out, const AV& av)
    {
        std::string tmp;
